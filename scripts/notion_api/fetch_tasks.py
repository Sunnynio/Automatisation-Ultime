...
Usage:
  ...
  python fetch_tasks.py --energie Faible                      # filtre par niveau d'énergie
  python fetch_tasks.py --support Téléphone --duree "30 min" --pays Thaïlande --energie Faible
...
def get_db_id(override=None):
    ...
    if not db_id:
        print("ERREUR : NOTION_DATABASE_ID manquant.", file=sys.stderr)
        sys.exit(1)
    return db_id
...
def filter_session(tasks, supports=None, durees=None, pays=None, energie=None):
    ...
    if energie:
        result = [t for t in result if prop(t, "🔋 Énergie", "select") in (energie, None)]

    def sort_key(t):
        u = URGENCE_ORDER.get(prop(t, "🚨 Urgence", "select"), 99)
        i = IMPORTANCE_ORDER.get(prop(t, "💡 Importance", "select"), 99)
...
def task_line(t, i=None):
    ...
    urgence = prop(t, "🚨 Urgence", "select") or ""
    importance = prop(t, "💡 Importance", "select") or ""
    energie = prop(t, "🔋 Énergie", "select") or ""
    ...
    details = " | ".join(x for x in [dur, urgence, importance, energie, sup, pays, ech] if x)
...
def print_prompt(tasks, mode, context=None):
    ...
    if mode == "digest":
        ...
        print(
            ...
            "Ordre de priorité : Urgence > Importance > Durée courte > Échéance."
        )
...
def main():
    ...
    elif args.support or args.duree or args.pays or args.energie:
        tasks = filter_session(all_tasks, args.support, args.duree, args.pays, args.energie)
